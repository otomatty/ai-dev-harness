...
技術選定は「情報を集める」作業ではなく「判断可能な形に情報を整理する」作業である。**Claude Code での実行を前提**とし、本線(オーケストレーター)は段取りと統合に徹し、重い調査はサブエージェントへ委譲して本線コンテキストを汚さない。

関連スキル/エージェント:
- 各候補の調査 → **candidate-researcher** サブエージェント(`{{HARNESS_DIR}}/agents/candidate-researcher.md`)
- 裏取り → **source-verification** スキル(サブエージェント側で適用)
- 成果物のHTML化 → **research-writeup** の生成器(`assets/render.py`)
...
2. 候補の洗い出しと絞り込み(本線)
3. 評価軸の設定(本線)
4. 候補ごとに candidate-researcher へ委譲(並列可)
5. 返ってきたJSON断片を統合(本線)
6. トレードオフの整理(本線)
7. research-writeup で原文引用つき図解HTMLを生成(本線)
8. フォローアップ(深掘り / 修正)への対応(本線)
...
- **「なぜ」/「もっと調べて」/「裏付けは?」(深掘り):** オーケストレーションのプロンプトと当該箇所を分析し、どの候補のどの論点かを特定して、その候補の candidate-researcher を**深掘りモード**で再委譲する。渡すのは対象論点のみ。返ってきた `items` を既存 findings にマージし、render.py で再生成する。複数箇所なら候補ごとに分けて委譲する。
